    ...
    <div class="pagina_presenti">
        ...
        while($record = gdrcd_query($result, 'fetch')) {
            ...
            if($ultimo_luogo_corrente != $luogo_corrente) {
                ...
                echo '<div class="luogo"><center>'.gdrcd_filter('out', $luogo_corrente).'</li></center><div class="lista_presenti_scroll">';
            } //if

            //Stampo il PG
            if(($record['is_invisible'] == 0) || ($record['nome'] == $_SESSION['login'])) {
                echo '<div class="presente" style="height: 14px; overflow: hidden; background: transparent !important;">';
               ...
               /* if($record['icon'] == '') {
                ...
                echo ' <a href="../main.php?page=scheda&pg='.$record['nome'].'" class="link_sheet nome_pg" target="_top">'.gdrcd_filter('out', $record['nome']);
...
if($record['numero'] == 1) {
    ...
    echo '<div class="page_title"><h2 class="presenti_title">'.$record['numero'].' '.gdrcd_filter('out', $MESSAGE['interface']['logged_users']['plur']).'</h2></div>';
}
echo '</a></div>';
